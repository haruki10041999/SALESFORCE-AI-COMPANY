import { z } from "zod";
import type { RegisterBatchToolsDeps } from "../register-batch-tools.js";

type BatchTopicConfig = {
  topic: string;
  agents?: string[];
  appendInstruction?: string;
};

export function defineBatchChatTool(deps: RegisterBatchToolsDeps): void {
  ...
  govTool(
    ...
    async ({ topics, topicConfigs, agents, persona, skills, maxContextChars, appendInstruction, parallel }: {
      ...
      const buildOne = async (cfg: BatchTopicConfig) =>
        buildChatPrompt(
          cfg.topic,
