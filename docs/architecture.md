...
- `mcp/core/`
  - `quality/`: zod を使った検証 / `agent-trust-score` (synergy bonus 対応)
  - `resource/`: リソース選択・提案・スコアリング / `embedding-ranker` (n-gram cosine hybrid) / `synergy-model` / `query-intent-classifier` / `usage-pattern` / `cascading-delete` / `cleanup-scheduler`
  - `governance/`: しきい値、上限、disable 状態管理
  - `event/`: イベント発火と履歴管理
  - `trace/`: トレース文脈管理 (`startPhase`/`endPhase`/`withPhase` で input/plan/execute/render を計測)
  - `learning/`: `rl-feedback` (Thompson sampling bandit) / `model-registry` (shadow/promote/rollback)
  - `observability/`: `dashboard` (HTML/Markdown/JSON 出力)
  - `context/`: `persona-style-registry` / `prompt-cache-persistence`
  - `logging/`: ログ出力制御
