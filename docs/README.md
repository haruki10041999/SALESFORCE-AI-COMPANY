# docs ランディング

このページは docs 配下の最短入口です。
詳しい索引は [documentation-map.md](./documentation-map.md) を参照してください。

## 初級者向け

- 用語を把握する: [non-engineer-glossary.md](./non-engineer-glossary.md)
- 開発を始める: [developer-guide.md](./developer-guide.md)
- 運用フローを確認する: [operations-guide.md](./operations-guide.md)

## 運用者向け

- 運用手順: [operations-guide.md](./operations-guide.md)
- SLA/SLO: [sla-slo.md](./sla-slo.md)
- DR フェイルオーバー: [dr-failover.md](./dr-failover.md)
- 指標評価: [metrics-evaluation.md](./metrics-evaluation.md)

## 深掘り設計

- 設計要約: [architecture.md](./architecture.md)
- 詳細 UML: [system-architecture-with-uml.md](./system-architecture-with-uml.md)
- プロンプト/リソース選択: [prompt-resource-logic.md](./prompt-resource-logic.md)
- テスト責務マップ: [test-file-responsibility-map.md](./test-file-responsibility-map.md)
